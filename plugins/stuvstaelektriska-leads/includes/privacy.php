...
add_filter(
	...
	function ( $exporters ) {
		$exporters['stuvstaelektriska-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'stuvstaelektriska-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'stuvstaelektriska-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'stuvstaelektriska-leads',
						'group_label' => __( 'Quote requests', 'stuvstaelektriska-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['stuvstaelektriska-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'stuvstaelektriska-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
