...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'stuvstaelektriska-leads' ),
					'singular_name' => __( 'Lead', 'stuvstaelektriska-leads' ),
					'edit_item'     => __( 'Förfrågan', 'stuvstaelektriska-leads' ),
					'search_items'  => __( 'Search leads', 'stuvstaelektriska-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'stuvstaelektriska-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'stuvstaelektriska-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'stuvstaelektriska-leads' ),
			'npl_phone'    => __( 'Phone', 'stuvstaelektriska-leads' ),
			'npl_service'  => __( 'Needs', 'stuvstaelektriska-leads' ),
			'npl_timeline' => __( 'When', 'stuvstaelektriska-leads' ),
			'npl_zip'      => __( 'ZIP', 'stuvstaelektriska-leads' ),
			'npl_source'   => __( 'Source', 'stuvstaelektriska-leads' ),
			'npl_status'   => __( 'Status', 'stuvstaelektriska-leads' ),
			'date'         => __( 'Received', 'stuvstaelektriska-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'stuvstaelektriska-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'stuvstaelektriska-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'stuvstaelektriska-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'stuvstaelektriska-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'stuvstaelektriska-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'stuvstaelektriska-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'stuvstaelektriska-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'stuvstaelektriska-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'stuvstaelektriska-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'stuvstaelektriska-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'stuvstaelektriska-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'stuvstaelektriska-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
